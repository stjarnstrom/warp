...
impl PaneGroup {
    ...
    fn restore_pane_leaf(
        ...
        let result = match leaf.contents {
            ...
            LeafContents::EnvVarCollection(_) => {
                anyhow::bail!("Environment variable panes can no longer be opened")
            }
            LeafContents::Workflow(_) => anyhow::bail!("Workflow panes can no longer be opened"),
    ...
    /// then focus it. Logs a warning if the pane is neither in the tree nor swap-hidden.
    ...
    /// Unlike `terminal_views()`, this excludes panes hidden for any reason.
