...
const SAFE_MODE_DESCRIPTION: &str = "Detect sensitive information in terminal blocks. Customize detection \
    with regular expressions and choose how detected secrets are displayed.";
...
const TELEMETRY_DESCRIPTION: &str = "App analytics help us make the product better for you. We only collect \
...
impl SettingsWidget for AppAnalyticsWidget {
    ...
    fn render(
        ...
        column.add_child(super::settings_page::build_toggle_element(
            ...
            ui_builder
                .switch(self.switch_state.clone())
                .check(privacy_settings.is_telemetry_enabled)
                .build()
                .on_click(|ctx, _, _| ctx.dispatch_typed_action(PrivacyPageAction::ToggleTelemetry))
                .finish(),
            appearance,
            None,
        ));
